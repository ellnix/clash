...
impl Renderer {
    ...
    fn render_command(&self, cmd: &Cmd) -> String {
        match cmd {
            ...
            Cmd::Write { text, output_text } => self.render_write(text, output_text),
    ...
    fn render_write(&self, text: &str, output_text: &str) -> String {
        let mut context = Context::new();
        let messages: Vec<&str> = text.lines().map(|msg| msg.trim_end()).collect();
        let output_messages: Vec<&str> = output_text.lines().map(|msg| msg.trim_end()).collect();
        context.insert("messages", &messages);
        context.insert("output_messages", &output_messages);
